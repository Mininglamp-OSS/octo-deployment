...
cd .. && ./setup.sh --verify                # 端到端自检（admin 登录 + 文件预签名 PUT）
...
#    COMPOSE_PROJECT_NAME 为你那份的后缀再 check：
...
- **客户端流量只需要一个 TCP 端口对外开**：`28080`（nginx HTTP，`OCTO_HTTP_PORT`）。启用 HTTPS 后客户端端口变成 `28443`（`OCTO_HTTPS_PORT`）。其他所有端口（MinIO、MySQL、Redis、WuKongIM monitor、各服务直连 REST）默认 loopback。WuKongIM 传输端口 `25001 / 25100 / 25200` 只在**原生 chat 客户端直连 WuKongIM** 时才需要外露；浏览器/SPA 的 chat 流量走 nginx `/ws`。
...
| octo-admin | `28082` | `0.0.0.0` | admin SPA（也通过 `/admin/` 访问） |
| octo-web | `28083` | `0.0.0.0` | user SPA（也通过 `/` 访问） |
| WuKongIM API / TCP / WS | `25001` / `25100` / `25200` | `0.0.0.0` | chat 客户端传输（仅在**原生 chat 客户端**直连 WuKongIM 时才需对外；浏览器/SPA chat 流量走 nginx `/ws`） |
