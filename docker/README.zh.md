...
`setup.sh --speech` 会自动生成 `SPEECH_DB_PASS`、`SPEECH_ADMIN_PASS`、`SPEECH_JWT_SECRET` 并写入 `docker/.env`。
随后执行 `sudo ./setup.sh --up` 时，脚本会先启动 `octo-speech-admin`，创建应用，并将获取到的 `SPEECH_API_KEY` 自动回写到 `docker/.env` —— 无需手动操作管理界面。
