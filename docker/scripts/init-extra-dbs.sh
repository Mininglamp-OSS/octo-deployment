...
#   - Accepted: printable ASCII bytes (0x20–0x7E) EXCEPT the three
#     forbidden characters above — including +, =, !, #, $, %, ^, &, *,
#     (, ), ~, `, {, }, [, ], |, ;, ", <, >, ,, ., ?, :, /, space, and
#     the always-safe [A-Za-z0-9._-]. This covers the output of
#     standard password generators (Terraform random_password,
#     1Password, openssl rand, etc.). Non-ASCII / multibyte bytes
#     (UTF-8 sequences, 0x80+) also pass validation but are not
#     generated by the password sources above.
...
validate_password() {
  ...
  # DSN (@). Use a POSIX case pattern instead of grep so the check is
  # portable across grep implementations (BSD grep treats \ inside
  # [...] differently from GNU grep — see security review on PR #144).
  # shellcheck disable=SC2102  # \\\\ is two-level escaping (shell + bracket), not a range
  case "$value" in
    *[\'\\\\@]*)
